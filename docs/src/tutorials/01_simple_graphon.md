...
This tutorial introduces graphons and shows how to use Graphons.jl to sample
random graphs. We'll cover the basic concepts, create simple graphon models,
and visualize the results.

## What is a Graphon?

A **graphon** (short for "graph function") is a mathematical object that
represents the limit of large random graphs. Formally, it's a symmetric,
measurable function:
...
Think of a graphon as a **continuous generalization** of a stochastic block
model (SBM). Instead of discrete blocks, every point in the unit square [0,1]²
...
1. **Assign latent positions**: For each node `i`, draw a random position ξᵢ ~
   Uniform[0,1]
2. **Sample edges**: For each pair of nodes `(i,j)`, the probability of an edge
   is:
...
This graphon creates graphs where nodes with higher latent positions (closer
to 1) are more likely to have edges. Nodes near 0 are sparse, while nodes near
1 form a dense subgraph.
...
The diagonal pattern shows that nodes with similar (and high) latent positions
are very likely to connect.
...
The simplest way is to use `rand`, which automatically draws random latent
positions:
...
Notice how edges cluster in the bottom-right corner? That's because nodes with
high latent positions (drawn randomly) tend to connect more densely.
...
With ordered latents, the structure is crystal clear! The density increases
smoothly from top-left (sparse) to bottom-right (dense).
...
- **Graphons** are continuous functions W: [0,1]² → [0,1] that generate random
  graphs
...
- For more on **Stochastic Block Models** with community structures and
  core-periphery patterns, see the next tutorial on Block Models
- For **multiplex networks** and graphs with rich edge attributes (weights,
  types, etc.), see the Decorated Graphons tutorial
